...
object KeyboardManager {
    ...
    // 轮19.57：坐标**已定稿**（按用户在 hint_offsets.txt 里实测调好的值写成默认），
    // 外置文件机制（hint_offsets.txt）已移除，避免以后两边不一致。
    // 语义：正值一律朝键面内侧（上/下 y = 距上/下边缘往里；左/右 x = 距左/右边缘往里，y 正数向下；
    //       长按 x = 距右边往里、y = 距顶边往里）。想微调用「恢复默认 + 参数」或直接找皓月改。

    /** 读取提示偏移（prefs 覆盖，缺省用内置默认值）。 */
    private fun hintOff(key: String, def: Int): Int = prefs.getInt("hint_off_$key", def)

    private fun hintOffX(key: String, def: Int): Int = hintOff(key, def).coerceIn(-80, 80)
    ...
    fun hintOffUpY(): Int = hintOffY("up_y", -5)
    ...
    fun hintOffDownY(): Int = hintOffY("down_y", -5)
    ...
    fun hintOffLeftX(): Int = hintOffX("left_x", 2)
    ...
    fun hintOffRightX(): Int = hintOffX("right_x", 2)
