...
| `oime-<版本>-armeabi-v7a.apk` | 32 位老设备 |

> 不确定选哪个：先装 `arm64-v8a`；若提示「应用未安装 / 不兼容」，再装 `armeabi-v7a`。
...
| **CPU** | **arm64-v8a** ✓ 与 **armeabi-v7a** ✓ —— **各自独立出包**（不合并 universal，单包更小）<br>x86 / x86_64 —— **暂不发布**（无设备测试）|
| 屏幕 | 竖屏 / 横屏（横屏为分体布局）|
