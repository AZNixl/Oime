...
- **单手模式**（左/右手，键盘缩到一侧）· **悬浮模式**（键盘缩小并可拖动整体移动）
- **长按可自定义（一个字段管两件事）**：键盘编辑器里填「长按」——单个动作/符号直接执行，
  空格分隔多个则是长按气泡；支持 `{text}{Left}` 语法
- **符号随中英切换**：键面长按符号与长按气泡会自动在中文标点 / ASCII 之间切换
  （h→`_`、m→`:`、`,`→`!`、`.`→`?`、k→ASCII 括号），全部符号页含「中文符号 / 英文符号」两类
